...
int deadband = 1;
int SteerCentreOffset = 0; //was -280
int PedalCentre = 550;
float revspd = 0.5;
...
Smoothed <int> SteeringFeedbackVal; //now FuseADC
...
void setup() {
  pinMode(LocRemSwPin, INPUT); //TODO force local?
  ...
  pinMode(SteeringFeedbackPin, INPUT); //now FuseADC
  pinMode(LeftPedalPin, INPUT);
  pinMode(RightPedalPin, INPUT);
  pinMode(DividerPin, INPUT);
  pinMode(CurrentSensePin, INPUT); //now ShuntADC
  ...
  for (int i = 0; i < 10; i ++) {
    ...
    ShuntADC.add(analogRead(CurrentSensePin));
  }
...
void loop() {
  ...
  if (currentMillisA - previousMillisA >= interval) { // check if inteval passed
    ...
    if (false) { //Forcing local mode - (was if (digitalRead(LocRemSwPin)) {)
      //Remote mode
